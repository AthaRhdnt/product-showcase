...
@section('content')
            ...
            <table class="min-w-full border-collapse table-fixed text-sm text-gray-800 dark:text-gray-300">
                ...
                <tbody>
                    <tr class="bg-gray-50 dark:bg-gray-900">
                        ...
                        @foreach ($compareProducts as $product)
                            <td class="border-t border-gray-200 dark:border-gray-700 p-4">Rp
                                {{ number_format($product->price) }}</td>
                        @endforeach
                    </tr>
                    <tr>
                        <th class="p-4 text-left font-medium bg-gray-50 dark:bg-gray-900">Category</th>
                        @foreach ($compareProducts as $product)
                            <td class="border-t border-gray-200 dark:border-gray-700 p-4">{{ $product->category->name }}
                            </td>
                        @endforeach
                    </tr>
                    @foreach ($allAttributes as $attribute)
                        <tr class="{{ $loop->even ? 'bg-gray-50 dark:bg-gray-900' : '' }}">
                            ...
                            @foreach ($compareProducts as $product)
                                @php
                                    $value =
                                        optional($product->attributes->firstWhere('id', $attribute->id))->pivot
                                            ->value ?? '-';
                                @endphp
    ...
    @if (session('compare_back_url'))
        <a href="{{ session('compare_back_url') }}"
            class="inline-block mt-6 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white px-5 py-2 rounded hover:bg-gray-300 dark:hover:bg-gray-600 transition">
            ← Back
        </a>
    @endif
@endsection
